...
public class SearchServiceImpl implements SearchService {
    ...
    private static final double MAX_LEMMA_FREQUENCY_PERCENT = 0.7; // порог, экспериментально
    ...
    public SearchResponse search(String query, String siteUrl, Integer offset, Integer limit) {
        ...
        for (Site site : sites) {
            ...
            List<Lemma> filtered = queryLemmas.keySet().stream()
                    .map(lemmaStr -> lemmaRepository.findByLemmaAndSite(lemmaStr, site).orElse(null))
                    .filter(Objects::nonNull)
                    .filter(l -> (double) l.getFrequency() / totalPages <= MAX_LEMMA_FREQUENCY_PERCENT)
